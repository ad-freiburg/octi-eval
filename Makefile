...
RNDR_DEG2 := $(patsubst %, render/$(GRIDSIZE)/deg2/%/res_$(METHOD).json, $(DATASETS))

RNDR_DEG2_DPEN := $(patsubst %, render/$(GRIDSIZE)/deg2-dpen/%/res_$(METHOD).json, $(DATASETS))

RNDR := $(patsubst %, render/$(GRIDSIZE)/base/%/res_$(METHOD).json, $(DATASETS))
...
render/$(GRIDSIZE)/deg2-dpen/%/res_$(METHOD).json: datasets/%.json
...
render/$(GRIDSIZE)/deg2/%/res_$(METHOD).json: datasets/%.json
	@printf "[%s] Schematizing $< to $@ ... \n" "$$(date -Is)"
	@mkdir -p $(dir $@)
...
render/$(GRIDSIZE)/base/%/res_$(METHOD).json: datasets/%.json
	@printf "[%s] Schematizing $< to $@ ... \n" "$$(date -Is)"
	@mkdir -p $(dir $@)
